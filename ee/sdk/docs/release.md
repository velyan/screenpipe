...
nested under `ee/sdk`, the clean public Swift distribution is a mirror repo
...
rsync -a --delete \
  ...
  /path/to/screenpipe/ee/sdk/ "$WORKDIR/sdk/"
...
- `ee/sdk/Cargo.toml` has `publish = false`.
...
```bash
cargo publish --dry-run --manifest-path ee/sdk/tauri/rust/Cargo.toml
```
