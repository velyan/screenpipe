...
fn is_loopback_moya_compat_endpoint(path: &str) -> bool {
    matches!(
        path,
        "/search"
            | "/vision/capture-active-window"
            | "/vision/latest-active-window"
            | "/vision/active-window/health"
    )
}

fn is_loopback_request(req: &axum::extract::Request) -> bool {
    req.extensions()
...
impl SCServer {
    ...
    pub async fn create_router(&self) -> Router {
        ...
        router
            ...
            .layer({
                // API auth middleware — when api_auth is enabled, protected requests
                // must include a valid bearer token. Moya's localhost sidecar contract
                // still calls /search and active-window endpoints without auth, so those
                // paths remain loopback-only compatibility exceptions.
