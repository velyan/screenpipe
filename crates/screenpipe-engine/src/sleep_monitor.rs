...
mod tests {
    ...
    static SLEEP_MONITOR_TEST_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    fn with_sleep_monitor_state_lock<T>(test_fn: impl FnOnce() -> T) -> T {
        let _guard = SLEEP_MONITOR_TEST_LOCK
            .lock()
    ...
    fn test_recently_woke_flag() {
        with_sleep_monitor_state_lock(|| {
            let original = RECENTLY_WOKE.swap(false, Ordering::SeqCst);
            assert!(!recently_woke_from_sleep());
            RECENTLY_WOKE.store(true, Ordering::SeqCst);
            assert!(recently_woke_from_sleep());
            RECENTLY_WOKE.store(false, Ordering::SeqCst);
            assert!(!recently_woke_from_sleep());
            RECENTLY_WOKE.store(original, Ordering::SeqCst);
        });
    }
    ...
    fn test_screen_is_locked_flag() {
        with_sleep_monitor_state_lock(|| {
            let original = SCREEN_IS_LOCKED.swap(false, Ordering::SeqCst);
            assert!(!screen_is_locked());
            SCREEN_IS_LOCKED.store(true, Ordering::SeqCst);
            assert!(screen_is_locked());
            SCREEN_IS_LOCKED.store(false, Ordering::SeqCst);
            assert!(!screen_is_locked());
            SCREEN_IS_LOCKED.store(original, Ordering::SeqCst);
        });
    }
